...
module.exports = {
	"data": {
        name: 'mlb',
        description: "Browse the MLB. (Powered by ESPN)",
        integration_types: [0, 1],
        contexts: [0, 1, 2],
        options: [
            {
                name: 'game',
                description: "Get info on a particular MLB game by team. (Powered by ESPN)",
                type: 1,
                options: [
                    {
                        "name": "team",
	...
	async execute(interaction) {
        ...
        switch (subCommand) {
            case 'game': {
                ...
                const teamGames = await fetch(`https://site.api.espn.com/apis/site/v2/sports/baseball/mlb/teams/${team}/schedule`)
                    .then(res => {return res.json()});
                const newestGame = teamGames['events'][getImportantGameIndex(teamGames.events)];
                const gameData = await fetch(`https://site.api.espn.com/apis/site/v2/sports/baseball/mlb/summary?event=${newestGame.id}`)
                    .then(res => {return res.json()});
